"""Run the frozen 36+16 Phase 10B-3G evaluation exactly once."""
    ...
    os.environ.update(
        {
            ...
            "QA_EVIDENCE_COMPLETION_ENABLED": "true",
            "QA_EVIDENCE_SELECTION_DIVERSITY_ENABLED": "true",
            "QA_EVIDENCE_COMPLETION_MAX": "2",
        }
    )
...
async def main() -> int:
    ...
    async with httpx.AsyncClient(base_url="http://127.0.0.1:8011", timeout=240) as client:
        for split in ("development", "validation"):
            ...
            runner = Phase10BaselineRunner(
                ...
                output_dir=ROOT / "runtime" / "phase10b3g" / f"eval-{split}",
                required_trace_keys=("grounding_audit", "completed_evidence", "accepted_completion"),
    ...
    (OUT / "final_run_summary.json").write_text(
        json.dumps(
            {
                ...
                "code_commit": "integration-pending",
                "config": {"mode": "naive", "top_k": 12, "chunk_top_k": 20, "rerank": False, "completion_max": 2, "conditional_completion": True},
            },
            ensure_ascii=False,
            indent=2,
        )
